...
//    public var width:                   CGFloat                     = 145.0
    public var width:                   CGFloat                     = 120.0
//    public var HeightView:              CGFloat                     = 280.0
    public var HeightView:              CGFloat                     = 350.0
    public var showFromRight:           Bool                        = false
    public var animationDuration:       CGFloat                     = 0.25
    public var itemSize:                CGSize                      = CGSize(width: 100.0, height: 100.0)
    ...
    public init(itemImages: [UIImage], colors: [UIColor]?, selectedItemIndices: NSIndexSet?){
        ...
        selectedIndices = selectedItemIndices != nil ? NSMutableIndexSet(indexSet: selectedItemIndices! as IndexSet) : NSMutableIndexSet()
        borderColors = colors
        images = itemImages
    ...
    public override func loadView() {
        ...
        tapGesture = UITapGestureRecognizer(target: self, action: #selector(FrostedSidebar.handleTap(recognizer:)))
    ...
    public func showInViewController(viewController: UIViewController, animated: Bool){
        if let bar = sharedSidebar{
            bar.dismissAnimated(animated: false, completion: nil)
        }
        
        delegate?.sidebar(sidebar: self, willShowOnScreenAnimated: animated)
        sharedSidebar = self
        
        addToParentViewController(viewController: viewController, callingAppearanceMethods: true)
        ...
        contentFrame.size.height = HeightView
        contentView.frame = contentFrame
        contentView.contentOffset = CGPoint(x: 0, y: 0)
        layoutItems()
        
        var blurFrame = CGRect(x: showFromRight ? view.bounds.size.width : 0, y: 0, width: 0, height: HeightView)
        ...
        let completion: (Bool) -> Void = { finished in
            if finished{
                self.delegate?.sidebar(sidebar: self, didShowOnScreenAnimated: animated)
            }
        }
        ...
        for (index, item) in itemViews.enumerated(){
            ...
            animateSpringWithView(view: item, idx: index, initDelay: animationDuration)
        }
    }
    ...
    public func dismissAnimated(animated: Bool, completion: ((Bool) -> Void)?){
        ...
        let completionBlock: (Bool) -> Void = {finished in
            self.removeFromParentViewControllerCallingAppearanceMethods(callAppearanceMethods: true)
            self.delegate?.sidebar(sidebar: self, didDismissFromScreenAnimated: true)
            self.layoutItems()
            if completion != nil{
                completion!(finished)
            }
        }
        self.delegate?.sidebar(sidebar: self, willDismissFromScreenAnimated: true)
        if animated{
            let parentWidth = view.bounds.size.width
    ...
    public func Taponitem(index : Int){
        self.didTapItemAtIndex(index: 0)
    }
    ...
    private func didTapItemAtIndex(index: Int){
        ...
        if index == 0 {
          //   NotificationCenter.default.post(name: Foundation.Notification.Name(rawValue: "AddShareButton"), object: nil)
            ...
            //NotificationCenter.default.post(name: Foundation.Notification.Name(rawValue: "RemoveShareButton"), object: nil)
        }
...
//        delegate?.sidebar(self, didTapItemAtIndex: index)
        self.delegate?.sidebar(sidebar: self, didTapItemAtIndex: index)
        delegate?.sidebar(sidebar: self, didEnable: didEnable, itemAtIndex: index)
    }
...
//                    item.layer.borderColor = borderColors![index].cgColor
    ...
    private func addToParentViewController(viewController: UIViewController, callingAppearanceMethods: Bool){
        if (parent != nil){
            removeFromParentViewControllerCallingAppearanceMethods(callAppearanceMethods: callingAppearanceMethods)
        }
